...
. link:power-sg120-and-sg1200-off-on.html#shut-down-the-sg120-or-sg1200-appliance["アプライアンスをシャットダウンします"]。
...
.. link:reinstalling-sg120-and-sg1200-into-cabinet-or-rack.html["障害が発生したアプライアンスをキャビネットまたはラックから取り外す"]。
.. 故障した機器における交換可能な部品（電源装置2台、冷却ファン7個、NIC3枚、SSD2枚）の位置を確認してください。
+
2つのドライブは、シャーシ内で次の位置にあります（ベゼルを取り外した状態のシャーシ前面）。

+
image::../media/sg120_sg1200_ssds_locations.png[ドライブの場所]
...
CAUTION: ドライブ上のデータを保持したい場合は、故障した機器で使用されていたのと同じドライブスロットにSSDドライブを挿入してください。そうしないと、Appliance Installerに警告メッセージが表示され、アプライアンスがグリッドに再接続できるようになるには、ドライブを正しいスロットに挿入してアプライアンスを再起動する必要があります。

.. link:reinstalling-sg120-and-sg1200-into-cabinet-or-rack.html["交換用アプライアンスをキャビネットまたはラックに設置する"]。
...
* link:../installconfig/troubleshooting-hardware-installation-sg120-and-sg1200.html#view-error-codes["アプライアンスのブート時のコードを確認します"]
