...
NOTE: アプライアンスのメンテナンスモード中にIP設定を変更した場合、インストールされているStorageGRID環境には適用されません。リンク../Maintain/changing-nodes-network-configuration.htmlを実行します。[`change-ip` Command）アプライアンスをStorageGRIDでリブートしたあとに、
