...
summary: SG6200-CNコントローラが最適に機能していない場合、または障害が発生している場合は、交換が必要になることがあります。 
---
= SG6200-CNコントローラの交換
:allow-uri-read: 
:icons: font
...
[role="lead"]
SG6200-CNコントローラが最適に機能していない場合、または障害が発生している場合は、交換が必要になることがあります。

.このタスクについて
SG6200-CNコントローラを交換すると、アプライアンスのストレージ ノードにアクセスできなくなります。SG6200-CNコントローラが正常に動作している場合は、この手順の開始時に制御されたシャットダウンを実行できます。
...
+
link:locating-sg6200-in-data-center.html["データセンターでコントローラを探します"]
...
. SG6200-CNコントローラが制御されたシャットダウンを可能にするのに十分な機能を有している場合、link:power-sg6200-off-on.html["SG6200-CNコントローラをシャットダウンします"]。
