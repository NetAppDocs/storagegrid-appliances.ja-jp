...
summary: SANtricity OS 08.42.20.00\（11.42\）以降を現在使用しているストレージコントローラでは、Grid Managerまたはメンテナンスモードを使用してアップグレードを適用できます。 
---
= Grid Manager を使用した SG6200 ストレージ コントローラ上の SANtricity OS のアップグレード
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
アップグレードを適用するには、Grid Managerまたはメンテナンスモードを使用できます。
...
** 1つ以上の*承認*ボタンを選択して、1つ以上のノードをSANtricity OSアップグレードキューに追加します。ステータスが標準でない場合、*承認*ボタンは無効になります。
+
...
これ以上の操作は必要ありません。
 ...
 a| 
このノードでは SANtricity OS のアップグレードは適用されません。
 a| 
StorageGRID システムで管理できるストレージコントローラがノードにありません。このメッセージが表示されているノードをアップグレードせずに、アップグレードプロセスを完了します。
 ...
 a| 
SANtricity OS ファイルはこのノードと互換性がありません。
 a| 
...
* link:upgrading-santricity-os-on-storage-controllers-using-maintenance-mode-sg6260.html["メンテナンス モードを使用した SG6200 コントローラでの SANtricity OS のアップグレード"]
